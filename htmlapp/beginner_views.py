...
from htmlapp.part1_views import setuser

@login_required
def python_beginner(request,part="1",ch="1"):
    ...
    return render(request,'./course/python_beginner'+part+'/Ch'+ch+'/python_beginner'+part+'_Ch'+ch+'_synopsis.html',locals())
...
def beginner_course(request,part="1",ch="1",num="1"):
    ...
    coname="python_beginner%s_Ch%s_%s"%(part,ch,num)
    ...
    if request.method == 'POST': # 傳送方式POST
        ...
        if tf >= 0:
            code.write("import matplotlib;matplotlib.use('agg');"+"from function import *"+";"+tes+"\nmatplotlib.pyplot.savefig('htmlapp/static/image/plot.svg')")
        else:
            code.write(tes)
        ...
        code.close()
        os.system('./run.sh > Anser')# Linux管理者 權限 執行 python3.5版本 的 code.py "> Anser"表示存檔至 Anser
        ...
        if anserr >= 0:
           ...
           mng="變數或套件錯誤"
        comparison=course4_detail.objects.get(name='python_beginner',part=part,ch=ch,num=num)
        ...
        if 'help' in request.GET:
            hel='help'
            helpmeg=course4_detail.objects.get(name='python_beginner',part=part,ch=ch,num=num)
    ...
    return render(request,'./course/python_beginner'+part+'/Ch'+ch+'/python_beginner'+part+'_Ch'+ch+'_'+num+'.html',locals())